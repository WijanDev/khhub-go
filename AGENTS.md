...
7. `POST /dev/reset-seed` is an explicit allowlist: `APP_ENV=development` or `APP_ENV=staging`. Production never registers the route (router drops the callback when `APP_ENV=production`). Staging loads the fictional demo seed on an empty directory and may reset that demo data; do not put real congregation PII in staging.
