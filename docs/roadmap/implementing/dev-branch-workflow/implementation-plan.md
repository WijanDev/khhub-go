...
2. Update `.github/workflows/ci.yml`: `pull_request` branches `[dev, main]`; `push` tests on `dev` and `main`. Keep the **images + Dokploy hook** job on `push` to `main` only (`github.ref == refs/heads/main`). Point Dependabot at `target-branch: dev`.
